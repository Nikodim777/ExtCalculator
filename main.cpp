...
BOOL CALLBACK DlgProc1(HWND hw, UINT msg, WPARAM wp, LPARAM lp);
...
LRESULT CALLBACK WindowProcedure (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
        ...
        if(cur_y < 55 && !strcmp(deft, "Введ")) {
            SetWindowText(edit, "");
            fenter = 1;
        }
        if(cur_y > 55 && !strcmp(deft, "")) {
            if(fwork) {
    ...
    case WM_COMMAND:
        ...
        case FUNCCALC:
            ...
            break;
        case FUNCINT:
            DialogBox(GetModuleHandle(0), MAKEINTRESOURCE(5), hwnd, DlgProc);
            in = fopen("lex.txt", "w");
            fprintf(in, "");
            fclose(in);
            GetWindowText(edit, expr, 256);
        ...
        case FUNCSAVE:
            ...
            break;
        case FUNCROOT:
            DialogBox(GetModuleHandle(0), MAKEINTRESOURCE(6), hwnd, DlgProc1);
            in = fopen("lex.txt", "w");
            fprintf(in, "");
            fclose(in);
            GetWindowText(edit, expr, 256);
